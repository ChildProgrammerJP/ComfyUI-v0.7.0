# 2149行目：カスタムノードのファイルが見つからない場合にエラーとならない処理に修正
...
async def load_custom_node(module_path: str, ignore=set(), module_parent="custom_nodes") -> bool:
    ...
    # カスタムノードのファイルがない場合にエラーとならない処理に修正 
    try:
        logging.debug("Trying to load custom node {}".format(module_path))
        
        # 1. sys_module_name が未定義の場合のバックアップ
        safe_module_name = sys_module_name if 'sys_module_name' in locals() else module_name

        # 2. パスの補正：指定されたパスがファイルでなくても、.py を付けると存在するか確認
        actual_path = module_path
        if not os.path.isfile(actual_path) and os.path.isfile(actual_path + ".py"):
            actual_path += ".py"

        # 3. 読み込み判定
        if os.path.isfile(actual_path):
            # --- 単体ファイル（.py）として読み込む ---
            module_spec = importlib.util.spec_from_file_location(safe_module_name, actual_path)
            module_dir = os.path.split(actual_path)[0]
        else:
            # --- フォルダとして読み込む（__init__.py の存在チェック） ---
            init_path = os.path.join(actual_path, "__init__.py")
            if os.path.exists(init_path):
                module_spec = importlib.util.spec_from_file_location(safe_module_name, init_path)
                module_dir = actual_path
            else:
                # ファイルでも有効なフォルダでもない場合はスキップ
                logging.warning(f"Skipping {actual_path}: No valid file or __init__.py found.")
                return False
